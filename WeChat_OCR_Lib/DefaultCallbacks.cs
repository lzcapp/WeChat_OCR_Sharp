...
internal class DefaultCallbacks {
    ...
    public DefaultCallbacks() {
        ...
        callbacks["kMMRemoteProcessLaunchFailed"] =
            new MMRemoteProcessLaunchFailedDelegate(DefaultRemoteProcessLaunchFailed);
    ...
    private void DefaultReadShared(uint request_id, IntPtr request_info, IntPtr user_data) {
        Debug.WriteLine(
            $"【{nameof(DefaultReadShared)}】被调用, request_id: {request_id}, request_info: {request_info}");
    }
