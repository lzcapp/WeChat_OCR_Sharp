...
public class FileHandling {
    public static string GetWeChatDir() {
        using var key = Registry.LocalMachine.OpenSubKey(SystemHandling.Is64BitOperatingSystem()
        ...
        var dir = key.GetValue("DisplayVersion") is not string displayVersion
            ? string.Empty
    ...
    public static string GetMmmojoDllDir() {
        ...
        if (!string.IsNullOrEmpty(wechatDir)) {
            ...
            if (File.Exists(dllPath)) {
                ...
                if (fileInfo.Length > 0) return wechatDir;
            }
        }

        wechatDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
            @"Tencent\WXWork\WeChatOCR\");
        ...
        switch (directories.Length) {
            ...
            default:
                foreach (var directory in directories) {
                    ...
                    if (File.Exists(dllPath)) {
                        var fileInfo = new FileInfo(dllPath);
                        if (fileInfo.Length > 0) return dllPath;
                    }
                }
    ...
    public static string GetWeChatOCRExePath() {
            ...
            default:
                foreach (var directory in directories) {
                    ...
                    if (File.Exists(ocrExePath)) {
                        ...
                        if (fileInfo.Length > 0) return ocrExePath;
                    }
                }
        ...
        if (string.IsNullOrEmpty(ocrExePath)) {
            ocrExePath =
                Directory.EnumerateFiles(searchPath, "WeChatOCR.exe", SearchOption.AllDirectories)
